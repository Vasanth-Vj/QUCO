...
const EditStockOutModel = ({ show, onClose, stockOutPoNo, stockOutOrder, stockOutId}) => {
  ...
  useEffect(() => {
    const fetchStockOutData = async () => {
      try {
        ...
        console.error("Error fetching stock out data:", error.response || error.message);
        setLoading(false);
  ...
  //     try {
  //       const response = await apiService.get(`/stockOut/${productId}`);
