...
const Brand = ({ searchQuery, isModalOpen, onClose }) => {
  ...
  //   { id: 1, brand: 'Nike', status: 'active' },
  //   { id: 2, brand: 'Adidas', status: 'inactive' },
  //   { id: 3, brand: 'Puma', status: 'active' },
  //   { id: 4, brand: 'Reebok', status: 'inactive' },
  //   { id: 5, brand: 'Under Armour', status: 'active' },
  //   { id: 6, brand: 'New Balance', status: 'inactive' },
  //   { id: 7, brand: 'Asics', status: 'active' },
  //   { id: 8, brand: 'Converse', status: 'inactive' },
  //   { id: 9, brand: 'Vans', status: 'active' },
  //   { id: 10, brand: 'H&M', status: 'inactive' },
  //   { id: 11, brand: 'Zara', status: 'active' },
  //   { id: 12, brand: 'Uniqlo', status: 'inactive' },
  // ]);
  ...
  const fetchAllBrands = async () => {
    try {
      ...
      setBrand(response.data); // Assuming response.data contains an array of brands
    } catch (error) {
  ...
  const handleStatusToggle = async (id) => {
    try {
      ...
      await apiService.put(`/brands/${id}`, { status: updatedStatus });
     // Update data locally
     setData(data.map(brand => (brand.id === id ? { ...brand, status: updatedStatus } : brand)));
    } catch (error) {
  ...
  const handleEditClick = async (id) => {
    try {
      ...
      const updatedData = data.map(brand => (brand.id === id ? styleToUpdate : brand));
      setData(updatedData);
      setEditIndex(id);
    } catch (error) {
  ...
  const handleSingleBrand = async() => {
    try {
      const response = await apiService.post("/brands/create", {brandName: singleBrands});
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentData.map((row, index) => (
              <tr key={row.id} style={{ maxHeight: '50px' }}>
                ...
                {editIndex === row.id ? (
                    <button onClick={() => handleSaveClick(index, row.id)}  className="bg-green-200 border border-green-500 px-2 py-1 rounded-lg flex">
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-full max-w-[35vw] h-screen max-h-[50vh] overflow-y-auto lg:overflow-hidden">
            <div className="py-2 flex flex-col">
              ...
              <div className="flex flex-col items-center">
                <p className="text-gray-400 font-bold mt-10">*For multiple brand feed use enter after each values"</p>
                <input
                ...
                {addedBrands.length > 0 ? (
                  <div className="flex flex-wrap mt-3">
                    ...
                    <span className="text-gray-500 text-xl">No brand entries</span>
                  </div>
                )}
                </div>
              </div>
