...
const PiecesPerOuterCarton = ({ searchQuery, isModalOpen, onClose }) => {
  const [data, setData] = useState([
    { id: 1, piecesPerOuterCarton: 'Short length', status: 'active' },
    { id: 2, piecesPerOuterCarton: 'medium length', status: 'inactive' },
    { id: 3, piecesPerOuterCarton: 'Full length', status: 'active' },
    { id: 4, piecesPerOuterCarton: 'cut', status: 'inactive' },
    { id: 5, piecesPerOuterCarton: 'loose', status: 'active' },
    { id: 6, piecesPerOuterCarton: 'Jersey', status: 'inactive' },
    { id: 7, piecesPerOuterCarton: 'Jersey', status: 'active' },
    { id: 8, piecesPerOuterCarton: 'Jersey', status: 'inactive' },
    { id: 9, piecesPerOuterCarton: 'Jersey', status: 'active' },
    { id: 10, piecesPerOuterCarton: 'Jersey', status: 'inactive' },
    { id: 11, piecesPerOuterCarton: 'Jersey', status: 'active' },
    { id: 12, piecesPerOuterCarton: 'Jersey', status: 'inactive' },
  ]);
  ...
  const filteredData = data.filter(item =>
    item.piecesPerOuterCarton.toLowerCase().includes(searchQuery.toLowerCase())
  ); 
  ...
  const handleInputChange = (e, index) => {
    const newData = [...data];
    newData[index].piecesPerOuterCarton = e.target.value;
    setData(newData);
  };
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50 w-full">
            <tr>
              ...
              <th className="px-2 py-3 text-left text-md font-bold text-black 2xl:w-[500px] xl:w-[450px] min-w-[200px]">No of pcs per outer carton</th>
              ...
              <th className="px-2 py-3 text-center text-md font-bold text-black uppercase w-8">
                <button onClick={handleDelete} className="text-red-500">
                  <img src={deleteIcon} alt="Delete" className='min-h-5 min-w-5' />
                </button>
              </th>
            </tr>
          </thead>
              ...
              <tr key={row.id} style={{ maxHeight: '50px' }}>
                ...
                <td className="px-3 py-3 whitespace-nowrap text-md text-left text-black 2xl:w-[500px] xl:w-[450px] min-w-[200px]">
                  {editIndex === startIndex + index ? (
                    <input
                      type="text"
                      value={row.piecesPerOuterCarton}
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-full max-w-[35vw] h-screen max-h-[50vh] overflow-y-auto lg:overflow-hidden">
            <div className="py-2 flex flex-col">
              ...
              <div className="flex flex-col items-center">
                <p className="text-gray-400 font-bold mt-10">*For multiple “Pcs Per Carton” feed use enter after each values</p>
                <input
                ...
                {addedStyles.length > 0 ? (
                  <div className="flex flex-wrap mt-3">
                    ...
                    <span className="text-gray-500 text-xl">No carton entries</span>
                  </div>
                )}
                </div>
              </div>
