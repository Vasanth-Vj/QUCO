...
const FullDescription = ({ searchQuery, isModalOpen, onClose }) => {
  const [data, setData] = useState([
    { id: 1, fullDescription: '100% Cotton mens T shirt with chest HD printed', status: 'active' },
    { id: 2, fullDescription: 'Polyster mens T shirt with chest HD printed', status: 'inactive' },
    { id: 3, fullDescription: 'Back AOP', status: 'active' },
    { id: 4, fullDescription: 'SD Print', status: 'inactive' },
    { id: 5, fullDescription: 'Pencil Fit', status: 'active' },
    { id: 6, fullDescription: 'Jersey', status: 'inactive' },
    { id: 7, fullDescription: 'Jersey', status: 'active' },
    { id: 8, fullDescription: 'Jersey', status: 'inactive' },
    { id: 9, fullDescription: 'Jersey', status: 'active' },
    { id: 10, fullDescription: 'Jersey', status: 'inactive' },
    { id: 11, fullDescription: 'Jersey', status: 'active' },
    { id: 12, fullDescription: 'Jersey', status: 'inactive' },
  ]);
  ...
  const handleInputChange = (e, index) => {
    const newData = [...data];
    newData[index].fullDescription = e.target.value;
    setData(newData);
  };
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50 w-full">
            <tr>
              ...
              <th className="px-2 py-3 text-left text-md font-bold text-black uppercase w-60">full Description</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentData.map((row, index) => (
              <tr key={row.id} style={{ maxHeight: '50px' }}>
                <td className="px-2 py-3 whitespace-nowrap text-md text-center text-black w-40">{startIndex + index + 1}</td>
                <td className="px-2 py-3 whitespace-nowrap text-md text-left text-black w-60">
                  {editIndex === startIndex + index ? (
                    <input
                      type="text"
                      value={row.fullDescription}
            ...
            <div className="py-2 flex flex-col">
              <div>
                <div className="flex justify-center">
                  <h2 className="text-2xl font-bold">Add Full Description</h2>
                  <button
              ...
              <div className="flex flex-col items-center">
                <p className="text-gray-400 font-bold mt-10">*For multiple “Full description” feed use enter after each values</p>
                <textarea
                ...
                {addedStyles.length > 0 ? (
                  <div className="flex flex-wrap mt-3">
                    ...
                    <span className="text-gray-500 text-xl">No description</span>
                  </div>
                )}
                </div>
              </div>
